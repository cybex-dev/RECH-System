        ...
        EntityEthicsApplication.getLatestComponents(pk)
                ...
                .forEach(component -> {
                    ...
                    if (reviewerFeedback.size() == 0){
                        ...
                        reviewerFeedback.forEach(s -> {
                            JsonElement element = new JsonParser().parse(s);
                            JsonObject asJsonObject = element.getAsJsonObject();
                            JsonArray ops = asJsonObject.getAsJsonArray("ops");
                            feedbackArray.addAll(ops);
                        });
                        JsonObject opsObject = new JsonObject();
                        opsObject.add("ops", feedbackArray);
                        String opsObjectString = opsObject.getAsString();
                        reviewData.put(component.getComponentId(), Collections.singletonList(opsObjectString));
                    }
                });
        return reviewData;
    }
